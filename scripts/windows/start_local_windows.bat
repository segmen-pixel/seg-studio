...
if "%SEG_HOST%"=="0.0.0.0" if not defined SEG_API_TOKEN (
  for /f "delims=" %%T in ('""%PYTHON_EXE%" "%REPO_ROOT%\scripts\_lan_token.py""') do set "SEG_API_TOKEN=%%T"
  if not defined SEG_API_TOKEN (
    echo [ERROR] Could not create the LAN access token.
    echo         The server refuses to serve the LAN unauthenticated, so it
    echo         will not start. Check that %REPO_ROOT%\projects is writable.
    exit /b 1
  )
  echo.
  echo  ============================================================
  ...
  echo   Stored in projects\runtime_settings.json -- delete the
  echo   "api_token" entry there to rotate it.
  echo.
)
...
REM ---- One log file per start -------------------------------
REM   A single fixed name cannot be reused safely on Windows. `>>` opens the
REM   file without sharing it for writing, and a training subprocess inherits
REM   the API's stdout handle -- so if the API dies while a run is in flight,
REM   the orphan keeps holding the log and every later start fails to open it.
REM   `start /B` swallows that failure, so the service simply never appeared
REM   and nothing said why. Stamping the name means a stale handle can never
REM   block a start.
for /f "delims=" %%S in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd_HHmmss"') do set "LOG_STAMP=%%S"
if not defined LOG_STAMP set "LOG_STAMP=run"
...
start "Seg-Studio Trainer API" /B cmd /c "%SEG_VENV%\Scripts\python.exe -m uvicorn apps.trainer_api.app.main:app --host %SEG_HOST% --port 8002 >> logs\windows\trainer_%LOG_STAMP%.log 2>&1"

echo [INFO] Starting serving API (port 8001, host=%SEG_HOST%)
start "Seg-Studio Serving API" /B cmd /c "%SEG_VENV%\Scripts\python.exe -m uvicorn apps.serving_api.app.main:app --host %SEG_HOST% --port 8001 >> logs\windows\serving_%LOG_STAMP%.log 2>&1"

REM ---- UI dev server (optional) -----------------------------
where npm >nul 2>nul
if errorlevel 1 goto :no_npm
echo [INFO] Starting Vite UI dev server (port 5173, host=%SEG_HOST%)
start "Seg-Studio UI Dev" /B cmd /c "npm --prefix apps\trainer_ui run dev -- --host %SEG_HOST% --port 5173 >> logs\windows\ui_dev_%LOG_STAMP%.log 2>&1"
goto :after_npm
:no_npm
...
echo  Logs        : %LOG_DIR%\*_%LOG_STAMP%.log
echo.
...
echo    SEG_API_TOKEN=...          Shared secret required for a LAN bind.
echo                               Generated and stored automatically if unset.
