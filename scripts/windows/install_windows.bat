...
if "%SKIP_SAM%"=="1" (
  ...
  echo          SAM-related features will be unavailable.
...
"%PYTHON_EXE%" -m pip install -r "%VENV_DIR%\requirements-notorch.txt" >> "%LOG_FILE%" 2>&1
...
"%PYTHON_EXE%" -m pip install -r apps\serving_api\requirements.txt >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
...
"%PYTHON_EXE%" -m pip install --force-reinstall --index-url https://download.pytorch.org/whl/%TORCH_CUDA_INDEX% torch torchvision >> "%LOG_FILE%" 2>&1
...
"%PYTHON_EXE%" -m pip install --upgrade "onnxruntime-gpu>=1.19.0" >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
  echo   [WARN] onnxruntime-gpu install failed. Inference will use CPU.
) else (
  echo   [INFO] onnxruntime-gpu installed.
)
goto :torch_done
...
"%PYTHON_EXE%" -m pip install torch torchvision >> "%LOG_FILE%" 2>&1
