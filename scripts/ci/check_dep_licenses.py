...
Fail-closed: a package whose license cannot be resolved (no PyPI
metadata, repeated fetch failure) fails the gate. After verifying the
license manually, add the package to the allowlist with a reason and a
verification pointer.

Exit codes: 0 = clean, 1 = denied license found, unresolvable license
metadata, or allowlist/file error.
"""
...
def main() -> int:
    ...
    for name in sorted(deps):
        ...
        for attempt in (1, 2, 3):
            ...
            except Exception as e:  # transient network errors get retries...
                if attempt == 3:
                    # ...but persistent failure fails the gate (fail closed).
    ...
    if unknown:
        print(f"::error::{len(unknown)} package(s) with unresolvable license metadata "
    ...
    if denied or unknown:
        return 1
    print("OK — no blocked licenses, all licenses resolved")
    return 0
