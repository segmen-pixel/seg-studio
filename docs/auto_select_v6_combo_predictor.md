...
| Arch one-hot | 3 | `simpleunet` / `stdc` / `deeplabv3plus` (retired in 0.9.7; the slot is kept so the packaged v6 model's feature order stays valid, and is always 0 at inference) |
...
the v6 bundle was trained on 37 internal research projects, so
