...
# Pick a device. "CPU" works on every Intel machine and is the safe default.
# "AUTO" hands the choice to the runtime, which prefers the GPU whenever
# Intel Graphics drivers are installed -- see the note under "Choosing a
# device" before combining it with an INT8 model.
DEVICE = "CPU"
...
- `"NPU"` — Core Ultra ("Meteor Lake" and newer). Lowest power; FP16
  recommended.
- `"AUTO"` — lets OpenVINO pick the fastest available device. On a machine
  with Intel Graphics drivers that means the iGPU, so `"AUTO"` and an INT8
  model are the one pairing to avoid: most iGPUs cannot run INT8 and the
  compile fails. Pair `"AUTO"` with FP16, or name the device outright.
