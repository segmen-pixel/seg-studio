...
**Windows (serving API only — ONNX inference, no training deps):**
```bat
scripts\windows\start_serving_api.bat
```
...
# To allow LAN access: set SEG_API_TOKEN, then pass --host 0.0.0.0 AND set
# SEG_HOST=0.0.0.0. Both matter. --host opens the port; SEG_HOST is what
# the startup check reads to decide whether a token is required. Without a
# token the server answers only clients on this machine, so --host on its
# own gets you an open port that returns 401 to everyone else.
```
...
| `SEG_HOST` | `127.0.0.1` | Bind address. Set to `0.0.0.0` for LAN access — startup then requires `SEG_API_TOKEN` |
| `SEG_API_TOKEN` | (empty) | Shared secret for LAN binds. Required when `SEG_HOST` is non-loopback; the launchers generate one on the first LAN start |
