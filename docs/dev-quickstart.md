...
# SEG_HOST=0.0.0.0. The startup check that refuses an unauthenticated
# off-box bind reads SEG_HOST, not uvicorn's --host, so --host on its own
# binds to the LAN without the check ever firing.
```
