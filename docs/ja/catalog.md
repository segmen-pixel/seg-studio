...
| 機能 | できること | バッジ |
|---|---|:---:|
| 学習モード | 通常学習 / クイック / 転移学習 / 個数カウント（インスタンス）を選んで開始 | 標準 |
| 🆒 **個数カウント** | 1 枚あたりの部品数を数える。既に描いたセグメンテーションマスクから学習 | 目玉 |
| タイル学習・推論 | 小さな物体を撮影時の解像度のまま扱う (縮小で潰れない) | 便利 |
...
| カウント API | `/count` でクラス別個数と個体ごとの矩形を返す | 便利 |
...
- 🚀 [はじめてのファーストラン手順](first-run-manual.md) — 最初の推論結果までの最短手順
- 📘 [はじめてのハンドブック](handbook.md) — 全 16 章の通しチュートリアル
- 📖 [ユーザーガイド（詳細版）](user-guide.md) — 機能リファレンス
