...
**Windows（Serving API のみ — ONNX 推論、学習用依存なし）:**
```bat
...
# LAN アクセスを許可する場合: SEG_API_TOKEN を設定したうえで、--host 0.0.0.0 と
# SEG_HOST=0.0.0.0 の両方を指定してください。認証なしの外部公開を拒否する起動時
# チェックが見るのは SEG_HOST であって uvicorn の --host ではないため、--host だけ
# だとチェックが働かないまま LAN に公開されます。
```
...
| `SEG_HOST` | `127.0.0.1` | バインドアドレス。LAN アクセスには `0.0.0.0` を設定してください（この場合 `SEG_API_TOKEN` が必須になります） |
| `SEG_API_TOKEN` | （空） | LAN バインド時の共有シークレット。`SEG_HOST` が非ループバックのときは必須で、未設定だと起動しません。起動スクリプトが初回 LAN 起動時に自動生成します |
...
3. ログにエラーが出ていないか確認してください。
   - Windows: `logs\windows\trainer_<timestamp>.log`（起動ごとに 1 本。`scripts\windows\status_windows.bat` が最新を表示します）
