...
- **Stride optimisation no longer buys a small score with a large bill.** After
  ...
  Patches scale with the inverse square of the stride, so on one run a 0.0015
  better F1 bought nine times the patches -- paid on every prediction for the
  life of that model. Starting from a 128-pixel patch, which auto-config does
  choose, the candidate list reaches sixteen times.

  A finer stride now has to beat the coarser one by more than 0.005 F1. The case
  stride optimisation was added for is untouched: on a project with 1.9%
  foreground the finer stride scored 0.9714 against 0.9176, ten times the
  margin. **A run trained from now on may keep a coarser stride than the same
  run would have kept before, and predicts faster for it.** The log names the
...
- **A run now records when it actually started.** `created_at` is when the run
  ...
  queue -- measured across one install at 23 of 96 rows more than a minute
  apart, worst case 397.8 minutes. Everything that wanted a start time was
  reading a reservation, and the summary card said "Started" while showing one.
