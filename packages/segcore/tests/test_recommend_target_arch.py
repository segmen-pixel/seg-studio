...
class TestTargetArchOverridesVoting:
    def test_pinned_arch_selected_over_majority(self):
        # 3 stdc donors would win the vote; pin says simpleunet (minority).
        library = [
            _profile("A", "stdc", best_f1=0.9),
            _profile("B", "stdc", best_f1=0.9),
            _profile("C", "stdc", best_f1=0.9),
            _profile("D", "simpleunet", best_f1=0.8),
        ]
        rec = recommend(_query(), library, top_k=5, target_arch="simpleunet")
        assert rec.target_arch == "simpleunet"
        assert rec.donor is not None
        assert rec.donor.arch == "simpleunet"
    ...
    def test_pinned_arch_picks_best_matched_donor(self):
        ...
        library = [
            _profile("older", "simpleunet", best_f1=0.7),
            _profile("closer", "simpleunet", best_f1=0.95),
        ]
        rec = recommend(_query(), library, top_k=5, target_arch="simpleunet")
        assert rec.donor is not None
        assert rec.donor.arch == "simpleunet"
...
class TestLegacyBehaviourPreserved:
    def test_without_target_arch_voting_still_runs(self):
        # 3 stdc vs 1 simpleunet — stdc wins the vote, donor is stdc.
        library = [
            _profile("A", "stdc", best_f1=0.9),
            _profile("B", "stdc", best_f1=0.9),
            _profile("C", "stdc", best_f1=0.9),
