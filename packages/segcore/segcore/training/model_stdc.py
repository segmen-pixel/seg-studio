...
"""STDC (Short-Term Dense Concatenation) segmentation network.

Lightweight architecture optimized for real-time inference.
Uses dense concatenation within blocks for efficient multi-scale features.
All normalization uses GroupNorm (CoreML/batch=1 compatible).

Reference: Fan et al., "Rethinking BiSeNet for Real-time Semantic Segmentation" (CVPR 2021)
"""
...
class _STDCBlock(nn.Module):
    """Short-Term Dense Concatenation block.

    Progressively halves channels through sequential convolutions,
    then concatenates all intermediate outputs for multi-scale features.
    """

    def __init__(self, in_ch: int, out_ch: int, stride: int = 1, n_steps: int = 4):
        super().__init__()
        assert out_ch % (2 ** (n_steps - 1)) == 0
        self.n_steps = n_steps

        # First conv handles stride and channel change
        mid = out_ch // (2 ** (n_steps - 1))
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_ch, mid, kernel_size=3, stride=stride, padding=1, bias=False),
            _norm_layer(mid),
            nn.ReLU(inplace=True),
        )

        # Subsequent convs halve channels each step
        self.steps = nn.ModuleList()
        for i in range(1, n_steps):
            ch_in = mid * (2 ** (i - 1))
            ch_out = mid * (2 ** i)
            if i == n_steps - 1:
                ch_out = mid * (2 ** (i - 1))  # last step keeps same width
        ...
        # 1x1 fusion to target out_ch
        _total_ch = mid * n_steps  # approximate; computed dynamically
        self.fusion = None  # set in first forward if needed
...
class STDCNet(nn.Module):
    """STDC-based segmentation network.

    4-stage encoder with dense concatenation blocks.
