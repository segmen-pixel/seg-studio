...
# Pinned upstream revision (audited 2026-07-22). Fetching a moving branch
# would let upstream silently change the code, license mix, and deps we
# execute; bump only after re-auditing the new revision. Keep in sync with
# segcore.auto_select.dino_features._DINOV2_HUB_REF.
_DINOV2_HUB_REF = "facebookresearch/dinov2:7764ea0f912e53c92e82eb78a2a1631e92725fc8"
...
def load_dinov2_teacher(
    ...
    Notes:
        ...
        ``torch.hub.load`` pinned to the audited upstream revision
        (``_DINOV2_HUB_REF``); we deliberately do not redistribute the
        upstream hub repo because recent versions carry non-commercial
        license fragments alongside the Apache code, and we do not track
        a moving branch because that would change the executed code and
        license mix without re-audit.
        For air-gapped deployment, place the Apache-2.0-licensed Python
        files under the pinned hub cache directory (or the legacy
        ``~/.cache/torch/hub/facebookresearch_dinov2_main/``) manually.
    """
    ...
    # the pinned source revision on demand from torch.hub. The cache dir
    # name embeds the ref, so probe the pinned dir first and fall back to
    # the legacy "_main" dir populated by older air-gapped installs.
    _hub_root = _Path.home() / ".cache" / "torch" / "hub"
    _pinned_dirname = _DINOV2_HUB_REF.replace("/", "_").replace(":", "_")
    _hub_cache = _hub_root / _pinned_dirname
    if not _hub_cache.exists():
        _hub_cache = _hub_root / "facebookresearch_dinov2_main"
    ...
    if _bundled.exists() and _hub_cache.exists():
        ...
        # Bundled weight is available; fetch the pinned source revision via
        # torch.hub (cached in ~/.cache/torch/hub/ on first run).
        model = _torch.hub.load(
            _DINOV2_HUB_REF, variant, pretrained=False,
        )
        state = _torch.load(str(_bundled), map_location="cpu", weights_only=True)
        model.load_state_dict(state, strict=True)
    else:
        ...
        model = _torch.hub.load(
            _DINOV2_HUB_REF, variant, pretrained=True,
        )
