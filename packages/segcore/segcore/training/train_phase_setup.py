...
def build_train_loader(dataset, batch_size: int, loader_kwargs: dict) -> DataLoader:
    """The one way a training DataLoader is constructed.

    Both the initial build and the rebuild after hard-negative mining go through
    here, so the two cannot drift. They did: the rebuild passed only
    batch_size / shuffle / num_workers / pin_memory / persistent_workers, so
    after the first mining round the loader silently lost the planner's
    prefetch_factor and, more importantly, worker_init_fn -- the hook that stops
    workers from initialising CUDA.
    """
...
class LoaderSetup:
    ...
    pin_memory: bool
    claim_ctx: Any | None
    #: The exact kwargs the train loader was built with. Anything that rebuilds
    #: it -- hard-negative mining does, so workers pick up the new centres --
    #: must pass these back through build_train_loader rather than assembling a
    #: shorter set. It used to assemble its own, dropping prefetch_factor and
    #: worker_init_fn and re-deriving persistent_workers, so a run changed its
    #: loader configuration partway through: the worker init is what keeps
    #: workers from initialising CUDA, and losing it after the first mining
    #: round is not something any log would report.
    train_loader_kwargs: dict = field(default_factory=dict)
...
def setup_device_and_loaders(
    ...
    train_loader = build_train_loader(train_ds, config.batch_size, _loader_kwargs)
    log_fn(
