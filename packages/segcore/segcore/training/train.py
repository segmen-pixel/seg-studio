...
from .train_phase_setup import build_train_loader, setup_device_and_loaders
...
def train(
    ...
    _train_loader_kwargs = _setup.train_loader_kwargs
    ...
    while epoch < effective_epochs:
        ...
        if should_tune_fg_patch_prob(
            did_eval=did_eval,
            epoch=epoch,
            warmup_epochs=warmup_epochs,
            annotation_patches_only=config.annotation_patches_only,
        ):
            # These hold whichever validation pass ran last: the monitoring
            # pass on a normal epoch, the report pass (full val set, configured
            # stride) on a final / extension-boundary epoch. That is deliberate.
            #
            # It looks like the comparability problem 8a420a7 fixed for
            # best_f1, and it is not. best_f1 is a SERIES compared across
            # epochs, so every entry has to be measured the same way. This is a
            # sign test between P and R read from the SAME call, and P-R does
            # not care which population produced it. Both passes derive P and R
            # identically from micro-averaged argmax TP/FP/FN; they differ only
            # in population and stride, and on both counts the report pass is
            # the better estimate -- all N images at the inference-matching
            # stride, against ceil(sqrt(N)) images (6 of 30, 29 of 800) chosen
            # for speed. Forcing the monitoring value here was tried and
            # reverted: it downgraded the boundary epochs to the noisy subset
            # to buy a consistency this controller does not need.
            ...
            if val_prec > 0 and val_rec > 0:
                ...
                new_prob = next_fg_patch_prob(old_prob, val_prec, val_rec)
                if new_prob != old_prob:
                    train_ds.fg_patch_prob = new_prob
                    log_fn(
                        f"  fg_patch_prob: {old_prob:.2f} -> {new_prob:.2f} "
                        f"(P={val_prec:.3f} R={val_rec:.3f} "
                        f"range=[{FG_PATCH_PROB_FLOOR:.2f},{FG_PATCH_PROB_CAP:.2f}])\n"
                    )
        ...
        if (
            ...
            try:
                ...
                if n_hn > 0:
                    ...
                    # Same kwargs as the initial build, through the same
                    # function: this used to assemble a shorter set and drop
                    # prefetch_factor and worker_init_fn.
                    train_loader = build_train_loader(
                        train_ds, config.batch_size, _train_loader_kwargs,
                    )
            except Exception as e:
                log_fn(f"  Hard negative mining skipped: {e}\n")
        # epochs_no_improve counts validation rounds, not epochs: it is
        # incremented inside run_validation_round, which evaluates every
        # epoch up to 10 and every fifth epoch after that. The patience is
        # in the same unit, deliberately -- what it bounds is how many
        # measurements may fail to improve, not how much wall clock passes.
        if (
            ...
            log_fn(
                f"Early stopping at epoch {epoch}: no improvement for {eval_state.epochs_no_improve} validation rounds "
    ...
    if use_sw and (run_dir / "model.pt").exists() and not stopped:
        ...
        # Calibrate the shipped threshold on the weights actually shipped.
        #
        # The threshold sweep runs during the final epoch, against whatever
        # weights the model held then -- which is usually NOT the best
        # checkpoint. train_config.json would otherwise carry a threshold tuned
        # for one set of weights next to a different set, and serving applies it
        # to model.pt: the exported model runs at somebody else's optimum. The
        # best checkpoint is loaded just above, so sweep it here, once, at the
        # optimized stride.
        _recal = evaluate_sliding_window(
            model, images_dir, masks_dir, val_ids,
        ...
        if _recal and "optimal_threshold" in _recal:
            _old = (val_threshold_info or {}).get("optimal_threshold")
            val_threshold_info = _recal
            log_fn(
                f"Threshold calibrated on the best checkpoint: "
                f"{_recal['optimal_threshold']:.3f} (final-epoch sweep said {_old})\n"
            )
        _release_cuda_memory(device)
