...
from .amp_policy import amp_supported
...
def _estimate_tile_batch(patch_size: int, num_classes: int, safety: float = 0.55) -> int:
    ...
    try:
        import subprocess
        # Explicit encoding: text=True would decode with the console code
        # page, and a decode error inside subprocess's reader thread arrives
        # here as empty output rather than an exception -- which would look
        # like a GPU with no free memory and silently fall back to the
...
# Smallest weight sum treated as real tile coverage. This is a divide-by-zero
# guard only: it must stay far below any weight sum a real tile can produce,
# because anything larger rescales the result instead of averaging it.
_MIN_BLEND_WEIGHT = 1e-6


def blend_accumulated_probs(accum: np.ndarray, count: np.ndarray) -> np.ndarray:
    """Divide Gaussian-weighted probability sums by their true weight sum.

    ``accum`` holds ``sum(p * w)`` and ``count`` holds ``sum(w)`` over the same
    tiles, so the blended probability is exactly ``accum / count``. Dividing by
    anything larger than ``count`` does not clamp the result -- it *rescales*
    it. The output then stops being a probability and starts depending on how
    densely the tiles happened to overlap.

    This used to read ``count = np.maximum(count, 1.0)``, and that floor was
    reached constantly rather than rarely. With patch 256 the blend sigma is
    ``patch_out / 4 = 32``, so at the serving default stride
    (``patch * 3 // 4`` = 192) *every* output pixel has ``sum(w) < 1.0``
    (measured minimum 0.3795) and every probability was scaled down by up to
    2.6x -- while stride 64, used by post-training threshold calibration,
    stayed above 1.0 and was left untouched. A threshold calibrated at one
    stride then wrecked recall at the other: on a real run mean foreground
    probability over GT read 0.618 instead of 0.954, and recall at the shipped
    threshold 0.680 fell from 0.957 to 0.298 while precision held at 0.99.

    Nothing raised an error and argmax never moved, because ``count`` is
    broadcast across all channels and a common positive scale factor cannot
    change which channel is largest. Only the calibrated threshold, the F1
    curve and ECE were poisoned -- exactly the numbers nobody re-derives.

    Stride decides how many times a pixel is looked at. It must never decide
    how confident the answer is.
    """
    return (accum / np.maximum(count, _MIN_BLEND_WEIGHT)).astype("float32")
...
def finalize_job(job_dict: dict) -> tuple[np.ndarray, np.ndarray]:
    ...
    # Average overlapping regions (true weight sum -- see blend_accumulated_probs)
    avg_probs = blend_accumulated_probs(accum, count)
...
def sliding_window_predict(
    ...
    # Same rule as training (segcore.training.amp_policy): Turing returns
    # all-NaN logits under autocast once the tile batch passes four, and this
    # path batches ten. Enabling AMP here while training ran in FP32 is what
    # made validation F1 read 0.0000 on healthy weights.
    _use_amp = amp_supported(device)
