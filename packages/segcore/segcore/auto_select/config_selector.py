...
def _patch_prior(combo_key: str, features: dict[str, float]) -> float:
    ...
    is_dlv3 = "deeplabv3plus_" in combo_key

    # Jig-fixed (low variance) + high contrast → DLv3+ does well
    if jig_score >= 0 and inter_var >= 0:
        if inter_var < 200 and fg_bg_contrast > 0.1:
            bonus += 0.03 if is_dlv3 else 0.0
        # High variance scenes → STDC mild preference
        elif inter_var > 1500:
