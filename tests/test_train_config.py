...
# ===================================================================
# postprocess_min_area: 0 is "measure it", not "off"
# ===================================================================
class TestPostprocessMinAreaSentinel:
    """The training form sends this field by omission when it is on auto.

    That only works because 0 -- the schema default, and what an omitted field
    resolves to -- means "measure a 6-sigma threshold from the masks". A user
    who types a number is asking for the opposite, so the auto pass has to
    leave an explicit value alone. Both halves are asserted here: the UI's
    auto/manual switch is only honest if they hold.
    """

    @staticmethod
    def _masks(tmp_path, n: int = 3):
        import numpy as np
        ...
        for i in range(n):
            ...
            arr[4:8, 4:8] = 1              # one component of 16 px
            arr[20:20 + i + 1, 20:22] = 1  # and one that varies in size
            stem = f"item{i}"
            Image.fromarray(arr, mode="L").save(masks_dir / f"{stem}.png")
            ids.append(stem)
        return masks_dir, ids

    def test_zero_asks_for_a_measured_threshold(self, tmp_path):
        masks_dir, ids = self._masks(tmp_path)
        config = _default_config(postprocess_min_area=0)
        result = _auto_tune_training(
            config, len(ids), lambda _m: None, masks_dir=masks_dir, train_ids=ids,
        )
        assert result.postprocess_min_area is not None
        assert result.postprocess_min_area >= 1

    def test_a_number_the_user_chose_is_not_measured_over(self, tmp_path):
        """Nothing else distinguishes "auto" from "the user typed 5"."""
        masks_dir, ids = self._masks(tmp_path)
        config = _default_config(postprocess_min_area=5)
        result = _auto_tune_training(
            config, len(ids), lambda _m: None, masks_dir=masks_dir, train_ids=ids,
        )
        assert result.postprocess_min_area is None
        assert config.postprocess_min_area == 5

    def test_a_negative_value_cannot_be_stored(self):
        """max(0, ...) in the constructor means it would land on the auto
        sentinel rather than below it -- worth knowing, since the form clamps
        to 1 for the same reason."""
        assert _default_config(postprocess_min_area=-3).postprocess_min_area == 0
