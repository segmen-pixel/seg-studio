...
#: Both id shapes that exist on disk: full UUIDs minted before ids were
#: shortened, and the 12-hex ids minted now. The startup sweep deletes the
#: directories it does not recognise as projects, so it has to know both --
#: a check that admitted only one shape would purge every project of the other.
_ID_SHAPES = pytest.mark.parametrize("id_shape", ["legacy-uuid", "short-hex"])


def _make_id(shape: str) -> str:
    import uuid

    from app.core.paths import new_id

    return str(uuid.uuid4()) if shape == "legacy-uuid" else new_id()
...
@_ID_SHAPES
def test_orphan_cleanup_ignores_tombstoned_dir(id_shape):
...
@_ID_SHAPES
def test_orphan_cleanup_purges_stub_dir(id_shape):
...
def test_orphan_cleanup_leaves_non_project_directories_alone():
    """PROJECTS_DIR holds more than projects, and the sweep deletes what it
    cannot adopt.

    .library is the only surviving copy of a deleted project's best-run
    weights, .gpu_locks coordinates device claims, and people make their own
    directories. None of them has project.json or recognised content, so each
    reaches the purge branch the moment the name check lets it through -- which
    is why that check is an allow-list of the id shapes we mint rather than a
    general "looks like a safe name" test.
    """
    import shutil as _shutil
    ...
    names = [".library", ".gpu_locks", "cloud-proj", "scratch notes"]
    made = []
    for name in names:
        d = PROJECTS_DIR / name
        if not d.exists():
    ...
    try:
        _cleanup_orphan_project_dirs()
        for name in names:
            assert (PROJECTS_DIR / name).is_dir(), f"{name} must survive the sweep"
    finally:
        for d in made:
...
@_ID_SHAPES
def test_orphan_cleanup_adopts_imported_project(id_shape):
