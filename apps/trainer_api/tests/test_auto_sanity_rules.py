...
class TestArchRule:
    ...
    def test_retired_arch_unchanged(self):
        # A retired arch label (deeplabv3plus, removed in 0.9.7) must pass
        # through the arch sanity rule untouched rather than error.
...
class TestMultipleRulesFire:
    ...
    def test_no_rule_fires_on_clean_default(self):
        # "arch": stdc is the sanity-rule-stable value (the rule only rewrites
        # simpleunet -> stdc), so a clean config fires nothing.
        cfg = {
            "arch": "stdc",
            "fg_patch_prob": 0.7,
