...
from app.core.paths import RUNS_DIRNAME, predictions_dir
from segcore.training.layout import PRED_DIRNAME


def _make_instance_run(pid: str, rid: str = "run-inst-1") -> Path:
    run_path = Path(os.environ["SEG_PROJECTS_DIR"]) / pid / RUNS_DIRNAME / rid
...
def test_instances_route_404_for_semantic_run(client, project_with_image):
    ...
    run_path = Path(os.environ["SEG_PROJECTS_DIR"]) / pid / RUNS_DIRNAME / "sem-run"
...
def test_ensure_generates_all_artifacts_with_dedup(client, project_with_image):
    ...
    preds = predictions_dir(run_path)
    assert (preds / f"{item}.png").exists()
    assert (preds / f"{item}.confidence.png").exists()
    score = json.loads((preds / f"{item}.score.json").read_text(encoding="utf-8"))
...
def test_failed_write_leaves_no_partial_artifacts(client, project_with_image):
    ...
    for sub in ("instances", PRED_DIRNAME):
        d = run_path / sub
...
def test_instance_onnx_export_404_for_semantic_run(client, project_with_image):
    pid, _item = project_with_image
    run_path = Path(os.environ["SEG_PROJECTS_DIR"]) / pid / RUNS_DIRNAME / "sem-exp"
    run_path.mkdir(parents=True, exist_ok=True)
...
def test_instance_onnx_export_single_class_leaves_mapping_null(
    ...
    fake_onnx = Path(os.environ["SEG_PROJECTS_DIR"]) / pid / RUNS_DIRNAME / \
