...
class TestRecipeApplication:
    def test_high_confidence_recipe_arch_is_applied(self, wire):
        ...
        monkeypatch.setattr(
            ...
            make_rc(_StubConfigRec("deeplabv3plus", source="ml", confidence="high")),
        )
        ...
        assert config["arch"] == "deeplabv3plus"
    ...
    def test_low_confidence_recipe_is_not_applied(self, wire):
        ...
        config = {"auto_config": True, "arch": "deeplabv3plus"}
        apply_auto_select_and_config(
            project_id="proj",
            config=config,
            prepared_dir=tmp_path,
        ...
        # Note: user arch is deeplabv3plus here so the (independent)
