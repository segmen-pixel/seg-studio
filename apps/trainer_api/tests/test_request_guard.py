...
def _v(**kw):
    # local_peer=True by default: every case below describes a tokenless
    # loopback install answering the browser on the same machine. A tokenless
    # server refuses a remote peer outright, which is its own case at the
    # bottom of this file rather than a variable in the CSRF table.
...
def test_missing_token_unauthorized():
    assert _v(configured_token="s", supplied_token="", local_peer=False) == "unauthorized"


def test_wrong_token_unauthorized():
    assert _v(configured_token="s", supplied_token="nope", local_peer=False) == "unauthorized"
...
def test_non_ascii_token_is_unauthorized_not_an_exception():
    ...
    # guess was unusual.
    assert _v(configured_token="s", supplied_token="パスワード",
              local_peer=False) == "unauthorized"


def test_non_ascii_cookie_is_unauthorized_not_an_exception():
    assert _v(configured_token="s", supplied_cookie="パスワード", method="GET",
              local_peer=False) == "unauthorized"
...
# ── a tokenless server is local-only, whatever it bound to ──────────────────

def test_tokenless_rejects_a_remote_peer():
    """The Host header is written by the caller; the peer address is not.

    `uvicorn --host 0.0.0.0` with neither SEG_HOST nor a token set does not
    trip require_token_for_nonlocal_bind, which resolves the interface from
    SEG_HOST and the persisted lan_access flag. Without this the only thing
    left between a LAN caller and the whole API was a Host header they get to
    choose.
    """
    assert _v(local_peer=False) == "unauthorized"
    # Claiming loopback in the header changes nothing.
    assert _v(local_peer=False, host_header="localhost:8002") == "unauthorized"
    # Nor does presenting no Origin, which otherwise skips the CSRF check.
    assert _v(local_peer=False, method="GET", origin_header="") == "unauthorized"


def test_tokenless_still_serves_the_local_browser():
    assert _v(local_peer=True) == "allow"


def test_token_protected_server_still_answers_a_remote_peer_with_the_token():
    assert _v(local_peer=False, configured_token="s3cret",
              supplied_token="s3cret") == "allow"
