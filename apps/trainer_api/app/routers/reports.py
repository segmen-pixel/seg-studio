...
def _report_order_key(item: ReportListItem) -> datetime:
    """When a report was generated, as one comparable instant.

    A value with no zone is UTC, the rule this API follows throughout -- and
    comparing one against a tz-aware one raises rather than sorting, so the
    list would 500 rather than come out in an odd order.
    """
...
def list_reports(project_id: str):
    ...
    for d in sorted(reports_root.iterdir()):
        ...
        try:
            ...
            continue
    # Newest first, by the timestamp inside the report rather than the name of
    # the directory holding it. That name is stamped in local time, and a local
    # stamp sorts against a UTC one by the size of the zone offset -- so a list
    # ordered by name reshuffles around the moment the stamp changed, and does
    # so in whichever direction the reader's zone runs. created_at is UTC on
    # every report ever written.
    items.sort(key=_report_order_key, reverse=True)
    return items
