...
from ..core.paths import prepared_dir, project_dir, read_run_model_name, resolve_run_path, run_dir
...
def model_search(
    ...
    if _ds_stats is None:
        _runs_dir = project_dir(project_id) / "training" / "runs"
        if _runs_dir.exists():
...
def optimize_run(project_id: str, run_id: str):
    ...
    new_path = run_dir(project_id, new_run_id)
