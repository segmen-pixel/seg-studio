...
def _resolve_model_info(project_id: str, run_id: str, backend: str = "onnx"):
    ...
    # Streaming path: keep the coarse stride so the frame rate holds.
    sw_stride = _load_run_sw_stride(run_path, realtime=True)
    use_sw = _should_use_sliding_window(patch_size, sw_stride, output_stride)

    # The live path must predict the same mask the Results tab does, and the
    # Results tab applies the run's calibrated threshold and suppresses
    # deactivated classes (prediction_engine.py). Live inference used to do
    # neither: it took a bare argmax, i.e. an effective threshold of ~0.5 on a
    # binary head, so it predicted strictly MORE foreground than the calibrated
    # operating point -- more false NG, larger regions -- and could report a
    # class the user had switched off.
    from ..core.run_config import _load_run_inference_threshold
    inference_threshold = _load_run_inference_threshold(run_path)
    active_class_ids = None
    if classes:
        _items = classes.get("classes") if isinstance(classes, dict) else classes
        if isinstance(_items, list):
            active_class_ids = [
                int(c["id"]) for c in _items
                if isinstance(c, dict) and c.get("id") is not None and c.get("active", True)
            ] or None
...
async def infer_single(
    ...
    result = await loop.run_in_executor(
        ...
        lambda: runtime.predict_one(
            ...
            inference_threshold=info.get("inference_threshold"),
            active_class_ids=info.get("active_class_ids"),
        ),
    )
