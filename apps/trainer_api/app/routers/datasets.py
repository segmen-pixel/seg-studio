...
async def import_annotate_zip(project_id: str, file: UploadFile = File(...), max_gb: float = 4.0):
    ...
    def _process_one(
    ...
    ) -> tuple[str, str, bytes, int, int, bool, bool, list[int], bytes | None]:
        """Returns (arc, bname, png_bytes, w, h, has_mask, has_fg, class_ids, mask_bytes)."""
        ...
        return (arc, bname, png_bytes, width, height, has_mask, has_fg, class_ids, mask_bytes_val)
    ...
    with lock:
        ...
        existing_names = {f.name.lower() for f in annotate_dir.iterdir() if f.is_file()} if annotate_dir.exists() else set()
        for arc, bname, png_bytes, width, height, has_mask, has_fg, class_ids, mask_bytes_val in processed:
