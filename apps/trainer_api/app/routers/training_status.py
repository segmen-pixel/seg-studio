...
from ..core.paths import project_dir, read_run_model_name, resolve_run_path, run_dir
...
def _has_any_model(run_path: Path) -> bool:
    return (run_path / "model.pt").exists()
...
def _discover_fs_runs(project_id: str) -> list[tuple[str, Path]]:
    ...
    Scans:
      - training/runs/<run_id>/model.pt   (normal runs)
    ...
    # Archived runs (training/archive_*/<run_id>/)
