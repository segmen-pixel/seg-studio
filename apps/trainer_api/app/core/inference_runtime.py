...
from .ort_infra import reclaim_released_vram
...
class InferenceRuntime:
    ...
    def __init__(
        ...
        #: When the stream session was last loaded or run, and how many frames
        #: are running on it right now. See stream_session_in_use().
        self._stream_last_used: float | None = None
        self._stream_in_flight: int = 0
    ...
    def start(self):
        ...
        # The sessions this runtime loads never enter _ort_session_cache, so
        # the ORT idle poller cannot see them unless it is told to ask. The
        # bound method registers *this* runtime rather than whatever happens to
        # be the singleton: a runtime built directly -- as the camera path did
        # before it moved to get_inference_runtime(), and as any test does --
        # would otherwise arm a release that then looked somewhere else.
        try:
            from .ort_infra import register_idle_release_hook
            register_idle_release_hook(self.release_stream_session_if_idle)
        except Exception:
            _logger.warning("stream session idle release could not be armed", exc_info=True)
    ...
    def stop(self):
        ...
        # The GPU workers took their sessions down with them when run()
        # returned; the stream session has no thread to end, so it is dropped
        # here. A stopped runtime holding a card is nobody's idea of stopped.
        try:
            from .ort_infra import unregister_idle_release_hook
            unregister_idle_release_hook(self.release_stream_session_if_idle)
        except Exception:
            _logger.warning("stream session idle release could not be disarmed", exc_info=True)
        self.release_stream_session()
    ...
    def _finalize_job(self, job: _Job):
        ...
        try:
            ...
            from segcore.training.sliding_window import blend_accumulated_probs
            avg_probs = blend_accumulated_probs(job.accum, job.count)
    ...
    def _ensure_stream_session(self, onnx_path: str, device_id: str, num_classes: int):
        ...
        try:
            ...
            pass
        self._stream_last_used = time.monotonic()
    ...
    @contextmanager
    def stream_session_in_use(self, onnx_path: str, device_id: str, num_classes: int):
        """Hand out the stream session and hold the idle release off meanwhile.

        The session is deliberately *used* outside _stream_lock: one call runs a
        whole sliding window over a frame, and holding the lock across that
        would serialise every other caller behind it. That is also why dropping
        self._stream_session mid-pass would free nothing -- the caller below
        still holds the reference it was handed -- while leaving the next frame
        to build a second session beside the first. Counting the passes in
        flight is what makes releasing the session safe.

        Yields ``(session, input_name, output_name)``.
        """
        with self._stream_lock:
            self._ensure_stream_session(onnx_path, device_id, num_classes)
            ...
            self._stream_in_flight += 1
            self._stream_last_used = time.monotonic()
        try:
            yield session, inp_name, out_name
        finally:
            with self._stream_lock:
                self._stream_in_flight -= 1
                self._stream_last_used = time.monotonic()

    def _drop_stream_session(self) -> None:
        """Forget the stream session. Caller holds _stream_lock."""
        self._stream_session = None
        self._stream_session_key = ""
        self._stream_num_classes = 0
        self._stream_input_name = ""
        self._stream_output_name = ""

    def release_stream_session(self):
        """Release the stream ORT session."""
        with self._stream_lock:
            self._drop_stream_session()
        reclaim_released_vram()
        _logger.info("Stream session released")

    def release_stream_session_if_idle(self, idle_seconds: float | None = None) -> bool:
        """Drop the stream session once it has sat unused long enough.

        This is the Live Inspection path: a camera run loads a session, the tab
        is closed, and nothing ever calls release_stream_session() -- so the
        card stayed occupied until the process ended or a training run came
        along and cleared a different cache. Returns True when it was released.
        """
        from .ort_infra import ORT_IDLE_RELEASE_SECONDS
        ...
        with self._stream_lock:
            if self._stream_session is None:
                return False
            if self._stream_in_flight > 0:
                return False
            last = self._stream_last_used
            if last is None:
                return False
            ...
            self._drop_stream_session()
        reclaim_released_vram()
        _logger.info("Released the stream ORT session after %.0fs idle", idle)
        return True
    ...
    def predict_one(
        ...
        t0 = time.perf_counter()
        with self.stream_session_in_use(onnx_path, device_id, num_classes) as (
            session, inp_name, out_name,
        ):
        ...
        # Apply the run's calibrated threshold through the SAME function the
        # Results tab and serving use. A bare argmax is an effective 0.5 on a
        # binary head, which over-detects relative to the calibrated operating
        # point -- the comment above promises this path matches the Results tab.
        from segcore.training.prediction_rules import prediction_from_probs
        pred = prediction_from_probs(probs_full, fg_threshold=inference_threshold).astype(np.uint8)
    ...
    def predict_frame(
        ...
        with self.stream_session_in_use(onnx_path, device_id, num_classes) as (
            ...
            timings["inference"] = round((time.perf_counter() - t0) * 1000, 2)
        # Same calibrated threshold as the Results tab (see predict_one).
        from segcore.training.prediction_rules import prediction_from_probs
        pred = prediction_from_probs(probs_full, fg_threshold=inference_threshold).astype(np.uint8)
        confidence = np.max(probs_full, axis=0).astype(np.float32)
