...
def _cuda_visible_devices_for(device: str) -> str | None:
    """CUDA_VISIBLE_DEVICES value pinning a child process to ``device``.

    "cuda:N" -> "N", bare "cuda" -> "0", "cpu" -> "" (hide every GPU so
    libraries fall back to CPU), unknown/empty -> None (leave env untouched).
    Must be applied before the child's first CUDA initialisation.
    """
...
def _instance_train_subprocess_worker(
    dataset_dir_str: str,
    run_dir_str: str,
    params: dict,
    logs_path_str: str,
    ...
    """Instance-mode (RF-DETR-Seg) training worker running in a child process.
    Same contract as _train_subprocess_worker: importable at this path for
    multiprocessing spawn; exit codes 0=success, 1=error, 2=OOM."""
    import os as _os
    import signal as _signal
    import sys as _sys
    from pathlib import Path as _Path
    ...
    # Pin the GPU the scheduler claimed BEFORE any CUDA initialisation —
    # rfdetr has no device parameter and would otherwise run on the library
    # default (cuda:0), diverging from the device lock on multi-GPU boxes.
    _visible = _cuda_visible_devices_for(str(params.get("device", "")))
    if _visible is not None:
        _os.environ["CUDA_VISIBLE_DEVICES"] = _visible
        _sys.stderr.write(f"[train-subprocess] CUDA_VISIBLE_DEVICES={_visible!r}\n")
        _sys.stderr.flush()
    # albumentations update check would phone PyPI on import (R2).
    _os.environ.setdefault("NO_ALBUMENTATIONS_UPDATE", "1")
    # rfdetr's rich summary tables print em-dashes; on Japanese Windows the
    # inherited cp932 stdout raises UnicodeEncodeError and kills the run
    # AFTER training succeeded. Force UTF-8 (replace on failure) up front.
    ...
    try:
        from segcore.instseg.train_rfdetr import train_instance
        train_instance(
            dataset_dir=_Path(dataset_dir_str),
            run_dir=_Path(run_dir_str),
            params=params,
            log_fn=log_fn,
            stop_flag=is_stopped,
        ...
        is_oom = "out of memory" in msg or ("cuda" in msg and "alloc" in msg)
        log_fn(
            f"Instance training {'OOM' if is_oom else 'error'}: {exc}\n"
            f"--- Traceback ---\n{_tb.format_exc()}\n--- End Traceback ---\n"
        )
        _sys.exit(_TRAIN_EXIT_OOM if is_oom else _TRAIN_EXIT_ERROR)
    except BaseException as exc:
        import traceback as _tb
        try:
            log_fn(f"Instance training failed (subprocess): {type(exc).__name__}: {exc}\n")
            log_fn(_tb.format_exc() + "\n")
        except Exception:
            _sys.stderr.write(f"Instance training failed (log_fn also failed): {exc}\n")
            _tb.print_exc(file=_sys.stderr)
        _sys.exit(_TRAIN_EXIT_ERROR)
