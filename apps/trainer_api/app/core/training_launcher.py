...
from .paths import classes_path, pretrained_model_path, project_dir, run_dir, write_json
    ...
    try:
        with get_train_guard(project_id):
            with Session(engine) as session:
                ...
                record = TrainingRun(run_id=run_id, project_id=project_id, status=initial_status)
                session.add(record)
