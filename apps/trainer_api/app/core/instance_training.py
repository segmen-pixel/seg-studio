...
def _required_vram_gib(model_size: str, batch: int) -> float | None:
    """Measured peak VRAM plus headroom for *model_size* at *batch*.

    None when the size carries no measurements (see _VRAM_REQUIRED_GIB); the
    table is keyed by the batches that were profiled, and a batch between two
    keys takes the requirement of the largest key at or below it.
    """
...
def _fit_batch_to_vram(batch: int, grad_accum: int, budget_gib: float,
    ...
    *budget_gib* is what the GPU will actually lend us, not its nameplate
    ...
    while batch > 2:
        required = _required_vram_gib(model_size, batch)
        if required is None or required <= budget_gib:
            break
        batch = max(2, batch // 2)
        grad_accum *= 2
    return batch, grad_accum
...
def _vram_shortfall_gib(model_size: str, batch: int, budget_gib: float) -> float | None:
    """How much *model_size* at *batch* overruns *budget_gib*, or None if it fits.

    _fit_batch_to_vram stops at batch 2 because rfdetr handles batch 1 poorly,
    so its result is "the smallest supported configuration", not "a
    configuration that fits". On a card too small for even that, the two differ
    and only this tells them apart.
    """
    required = _required_vram_gib(model_size, batch)
    if required is None or budget_gib <= 0 or required <= budget_gib:
        return None
    return required - budget_gib
...
def run_instance_phases(
        ...
        if model_size in _VRAM_FITTED_SIZES:
            try:
                ...
                if fitted_batch != batch:
                    log_fn(f"[instance] VRAM dry-run: batch {batch}->{fitted_batch}, "
                           f"grad_accum {grad_accum}->{fitted_accum} "
                           f"(effective batch kept)\n")
                    batch, grad_accum = fitted_batch, fitted_accum
                short_by = _vram_shortfall_gib(model_size, batch, budget_gib)
                if short_by is not None:
                    # Nothing left to reduce: batch is at the floor of 2 and the
                    # smallest size on offer still asks for more than the policy
                    # budget allows. Worth recording, because it is the state a
                    # later CUDA OOM will have come from. Not worth predicting
                    # failure over.
                    #
                    # The budget is deliberately pessimistic -- a flat 2 GiB
                    # WDDM headroom taken off a card that only has 4 -- so being
                    # over it is not the same as not fitting. Measured on the
                    # GTX 1650 Max-Q this check calls 1.8 GiB short: RF-DETR-Seg
                    # small at batch 2 trains at 5.0 min/epoch, SM utilisation
                    # around 55% through the epoch, PCIe under 50 MB/s the whole
                    # way. It fits, and it is not paging.
                    #
                    # An earlier version of this message asserted the opposite,
                    # that Windows silently pages the excess into system RAM and
                    # that this was why epochs took hours. The hours were real;
                    # the cause was fp16 autocast on a Turing card, which
                    # amp_policy now gates. Do not restore the paging claim
                    # without a PCIe measurement behind it.
                    log_fn(f"[instance] VRAM dry-run: {model_size} at batch {batch} is "
                           f"the smallest configuration on offer, and it is "
                           f"~{short_by:.1f} GiB over the {budget_gib:.1f} GiB this "
                           f"policy budgets for the card. The budget is conservative "
                           f"on small GPUs and training often still fits; if it does "
                           f"raise CUDA OOM, this is why.\n")
            ...
            # _VRAM_REQUIRED_GIB).
            log_fn(f"[instance] VRAM dry-run: WARNING - no measurements for "
    ...
    params = {
        ...
        # Written in epochs here and converted to evaluations by
        # train_rfdetr.early_stopping_evals. Shared with the semantic path,
        # which reads the same key: instance runs used to ignore it and always
        # spend the full epoch count, on a fine-tune that typically reaches
        # its plateau in single-digit epochs.
    ...
    from segcore.instseg.train_rfdetr import (
        read_epoch_val_metrics,
        write_run_contract,
    )
    ...
    if stop_event.is_set():
        ...
        # The terminate above killed the child inside model.train(), so the
        # tail that marks the run usable never ran there. Write it from here:
        # the epochs the user already paid for are on disk as
        # checkpoint_best*.pth, and without the contract every one of them is
        # invisible to inference.
        try:
            kept = write_run_contract(dataset_dir, run_path, params, log_fn)
        except Exception as err:
            # Calibration loads the model back onto the GPU and can fail on
            # its own. A stop that cannot measure the threshold should still
            # keep the model rather than discard the run.
            log_fn(f"[instance] contract: calibration failed ({err}) — keeping "
                   f"the run with an uncalibrated threshold\n")
            try:
                kept = write_run_contract(dataset_dir, run_path, params, log_fn,
                                          calibrate=False)
            except Exception as err2:
                log_fn(f"[instance] contract: not written ({err2}) — this run "
                       f"keeps no usable model\n")
                kept = False
        log_fn("[instance] stopped run kept: the best checkpoint so far is "
               "usable for inference\n" if kept else
               "[instance] stopped before the first evaluation — no checkpoint "
               "to keep\n")
        return
