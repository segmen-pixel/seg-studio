...
def _run_auto_label(project_id: str, item_id: str, img_path: str,
    ...
    # GrabCut's initGMMs needs BOTH populations; seeding only one of them
    # aborts inside OpenCV with an assertion instead of a usable message.
    if not np.any((grab_mask == cv2.GC_FGD) | (grab_mask == cv2.GC_PR_FGD)):
        if ann_small is not None and has_spatial:
            grab_mask[(ann_small == class_id)] = cv2.GC_FGD
        else:
            raise ValueError(
                "Auto-label found no region resembling the painted class in this "
                "image. Paint a little of the class here first, or annotate more "
                "images so the colour model has something to match.")
    if not np.any((grab_mask == cv2.GC_BGD) | (grab_mask == cv2.GC_PR_BGD)):
        # Everything scored as foreground (a colour model that matches the
        # whole frame). The border is background in practice, so seed it and
        # let GrabCut pull the real boundary in from there.
    ...
    if not np.any((grab_mask == cv2.GC_BGD) | (grab_mask == cv2.GC_PR_BGD)):
        raise ValueError(
            "Auto-label could not tell foreground from background in this "
            "image — the painted class matches the whole frame. Paint some "
            "background, or narrow the class strokes, and try again.")
    ...
    try:
        cv2.grabCut(work_img, grab_mask, None, bgd_model, fgd_model, iters,
                    cv2.GC_INIT_WITH_MASK)
    except cv2.error as exc:
        # Degenerate seeding OpenCV rejects internally: report it as a user
        # error with something actionable rather than a 500.
        raise ValueError(
            "Auto-label could not segment this image from the current "
            "annotations. Paint a little more of the class (and some "
            "background) and try again.") from exc
