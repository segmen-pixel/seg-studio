...
def instance_artifact_paths(
    ...
    pred_dir = predictions_dir(run_path)
    if ensure_dir:
...
def ensure_instance_highlight_overlay(run_path: Path, item_id: str) -> Path:
    ...
    with _item_lock(run_path, item_id):
        ...
        # The source image lives under the project's annotate dir. Resolved
        # by walking up to PROJECTS_DIR rather than by hop count, which
        # differs between live and archived runs.
        project_id = project_id_of(run_path)
