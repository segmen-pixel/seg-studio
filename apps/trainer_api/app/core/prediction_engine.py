        ...
        if backend == "onnx":
            ...
            try:
                probs_np, _t_infer_ms, ort_provider_name = _predict_with_ort(
                    ...
                    inactive_class_ids=inactive_class_ids,
                    active_class_ids=active_class_ids,
                    _preloaded_session=_preloaded_session,
                )
                # If CUDA was requested but ORT fell back to CPU, prefer torch GPU
                if requested_predict_device.startswith("cuda") and ort_provider_name == "cpu":
                    import logging as _log
                    _log.getLogger(__name__).info(
                        "ORT fell back to CPU despite CUDA request; using torch GPU instead"
                    )
                else:
