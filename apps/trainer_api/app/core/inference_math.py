...
from .ort_infra import ort_session_in_use
...
def _ort_run_logits(session: object, input_name: str, output_name: str, batch_np: np.ndarray, num_classes: int) -> np.ndarray:
    ...
    # Every cached-session inference funnels through here, which makes this the
    # place that knows the sessions are busy. Without it the idle poller reads
    # the clock from whenever a session was last handed out, and a long batch
    # reusing one preloaded session looks idle the whole way through.
