...
class _GpuWorker(threading.Thread):
    ...
    def __init__(
        ...
        #: Batch size profiled per session key, kept across a release.
        self._profiled_batch: dict[str, int] = {}
        #: When this worker's session was last loaded or run, for the idle
        #: release below. None while there is no session to release.
    ...
    def _ensure_session(self, onnx_path: Path, device_id: str, num_classes: int):
        ...
        self._max_patches_per_call = self._resolve_max_batch(key)
    ...
    def release_session_if_idle(self, idle_seconds: float | None = None) -> bool:
        """Drop this worker's ORT session once it has sat unused long enough.

        Called from run() and from nowhere else. This worker thread is the only
        thread that ever runs this session, so on this thread "not busy" is a
        fact rather than a guess -- there is nothing to count and nothing to
        lock. An outside poller would have to null the session out from under a
        running _run_batch, which frees nothing (the batch below still holds the
        reference) and leaves the next batch to build a second session beside
        the first. On a 4 GB card that is the failure this exists to prevent.

        The session comes back on demand: the next batch calls _ensure_session,
        which reloads it. The profiled batch size is remembered across this, so
        what the reload costs is the load itself and not another search.
        """
        from .ort_infra import ORT_IDLE_RELEASE_SECONDS, reclaim_released_vram

        threshold = ORT_IDLE_RELEASE_SECONDS if idle_seconds is None else idle_seconds
        if threshold <= 0 or self._session is None:
            return False
        ...
        _logger.info(
            "GpuWorker %s: released its ORT session after %.0fs idle", self.device_id, idle,
        )
        return True
            ...
            try:
                ...
                while len(pending) < self.target_batch:
                    ...
                    if item is _SHUTDOWN:
                        # Flush remaining. Through _run_batch_with_meta rather
                        # than _run_batch: the session may have been released
                        # while the queue was empty, and _run_batch returns
                        # without a word when there is none, which would drop
                        # these chunks instead of failing them.
                        if pending:
                ...
                if not pending:
                    # The queue came back empty, so nothing is running on this
                    # thread and the session -- if there still is one -- is
                    # idle by definition. in_q.get() blocks for 2s when there
                    # is nothing pending, which makes this the poll the release
                    # needs without a thread of its own.
    ...
    def _resolve_max_batch(self, key: str) -> int:
        """Profile the batch size, or recall what this key profiled before.

        The search runs real inferences until they fail: 30.6s for a 512x512
        model on an RTX 3090, measured. Paying that once per process was fine
        while a session was loaded once per process, but the idle release makes
        reloads routine -- and 30s in front of the first prediction after a
        quiet spell is worse than the VRAM the release hands back.

        Recalling the number is safe because it was never a promise. _run_batch
        halves the batch and retries whenever an allocation fails, so a card
        that is tighter than it was when the search ran corrects itself on the
        first sub-batch, and stores the smaller number here on the way past.
        """
        remembered = self._profiled_batch.get(key)
        if remembered is not None:
            _logger.info(
                "GpuWorker %s: reusing the profiled batch size %d for %s",
                self.device_id, remembered, key,
            )
            return remembered
        profiled = self._profile_max_infer_batch()
        self._profiled_batch[key] = profiled
        return profiled
    ...
    def _run_batch(self, chunks: list[_Chunk]):
        ...
        try:
            ...
            while pos < total:
                ...
                try:
                    ...
                    if "allocate" in str(oom_exc).lower() and max_b > _MIN_BATCH_SIZE:
                        ...
                        # So a reload does not walk back into the same OOM.
                        self._profiled_batch[self._session_key] = max_b
                        _logger.warning(
            ...
            # Stamped on the way out rather than on the way in: one batch of
            # large patches can run for longer than the idle threshold, and a
            # stamp from before it would make the session eligible the instant
            # it returned -- having been in use for every second of it.
            self._last_used = time.monotonic()
