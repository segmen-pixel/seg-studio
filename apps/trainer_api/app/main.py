...
# albumentations (pulled by rfdetr[train]) checks PyPI for updates at import
# time; disable so the app never phones home (R2, instance-seg design §8).
...
# ---------------------------------------------------------------------------
# Fail fast if this process would serve an unauthenticated API off-box.
require_token_for_nonlocal_bind()
...
app.add_middleware(
    ...
    allow_headers=["Content-Type", "Accept", "Authorization", "X-API-Token"],
)
...
# Sign-in routes: how a browser with no credential yet obtains one, so they
# cannot themselves require a credential. `/auth/status` discloses only whether
# a token is required; `/auth/session` issues a cookie only to a caller that
# already knows the token. See routers/auth.py.
_UNGUARDED_PATHS = frozenset({
    API_V1_PREFIX + "/auth/status",
    API_V1_PREFIX + "/auth/session",
...
def _is_guarded_path(path: str) -> bool:
    ...
    Static UI assets, `/health`, `/docs`, `/openapi.json`, the sign-in routes,
    and CORS preflight requests stay open so the browser UI can still boot
    before the user has a token. Anything that mutates project state or exposes
    inference output is behind one of these prefixes.
    """
...
# Always-on request guard. When a token is configured it is required; when it
# is not (the loopback default), the guard still rejects cross-origin state
# changes and rebound Host headers, so a page the user visits cannot drive the
# local API. HTTP is handled here; WebSocket handshakes bypass "http"
# middleware and are judged by the same decision inside WebSocketTokenGate.
...
async def guard_requests(request: Request, call_next):
    if _is_guarded_path(request.url.path):
        verdict, reason = evaluate_request_guard(
            ...
            # A browser cannot put a custom header on an <img> or a download,
            # so the UI authenticates with the cookie issued by /auth/session.
            supplied_cookie=request.cookies.get(SESSION_COOKIE_NAME, ""),
            # Binding to the LAN should not make this machine's own browser
            # sign in to its own desktop app; a local peer is judged by the
            # same rules as the default loopback install.
            local_peer=is_local_peer(
                request.client.host if request.client else None, request.headers.keys()
            ),
        )
...
# Sign-in routes are registered eagerly rather than through the deferred
# registry: the registry skips a router whose import fails, and a silently
# missing /auth/session would lock every browser out of a token-protected
# server with no way back in. These have no optional dependencies to defer.
from .routers import auth as _auth_router  # noqa: E402

app.include_router(_auth_router.router, prefix=API_V1_PREFIX)
...
def _background_startup() -> None:
    ...
    try:
        ...
        # --- Windows path budget ---
        try:
            _check_path_budget(_startup_state)
        except Exception as exc:
            logger.warning("path budget check failed: %s", exc)
