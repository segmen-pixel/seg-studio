...
class TrainRunRead(BaseModel):
    ...
    created_at: datetime
    #: When training began. None on a run that is still queued, and on rows
    #: written before the column existed -- a reader that needs something to
    #: show falls back to created_at and says so.
