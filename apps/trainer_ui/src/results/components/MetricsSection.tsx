...
type Props = {
  ...
  /** Instance run (v0.9.8): render the segm-mAP/count subset instead of F1/mIoU. */
  isInstance?: boolean;
...
export default React.memo(function MetricsSection({
  ...
  if (activeRunId && scoreCacheRef.current) {
    ...
    for (const [key, score] of scoreCacheRef.current.entries()) {
      ...
      // Skip presence-only stubs: they carry class chips, not measurements,
      // and averaging their zeros dragged the reported mean toward 0.
      if (score.presence_only) continue;
  ...
  // ── Instance runs: segm mAP / AR / exact-count subset + dataset stats ──
  if (isInstance) {
    const numOrNull = (key: string): number | null =>
      m && typeof m[key] === "number" ? (m[key] as number) : null;
    const segmMap = numOrNull("segm_mAP_50_95_val");
    const segmAp50 = numOrNull("segm_mAP_50_val");
    const arVal = numOrNull("AR_val");
    const countExact = numOrNull("count_exact_val");
    const countN = numOrNull("count_exact_val_n");
    const instEpoch = numOrNull("best_epoch");
    const stats = (m?.["dataset_stats"] ?? null) as Record<string, unknown> | null;
    if (segmMap == null && countExact == null && !stats) return null;
    return (
      <div className="section" style={{ marginTop: 12 }} data-testid="instance-metrics">
        <div className="section-title">{t("results.metrics.instanceTitle")}</div>
        <div className="score-grid">
          <span className="score-label">mAP</span>
          <span className="score-val best">{segmMap != null ? segmMap.toFixed(3) : "—"}</span>
          <span className="score-label">AP50</span>
          <span className="score-val">{segmAp50 != null ? segmAp50.toFixed(3) : "—"}</span>
          <span className="score-label">AR</span>
          <span className="score-val">{arVal != null ? arVal.toFixed(3) : "—"}</span>
          <span className="score-label">Ep</span>
          <span className="score-val">{instEpoch != null ? String(instEpoch) : "—"}</span>
        </div>
        {countExact != null && countN != null && countN > 0 && (
          <div className="muted" style={{ fontSize: 11, marginTop: 6 }}>
            {t("results.metrics.countExact")
              .replace("{ok}", String(countExact))
              .replace("{n}", String(countN))}
          </div>
        )}
        {stats && (
          <div className="muted" style={{ fontSize: 10, marginTop: 4 }}>
            {t("results.metrics.instanceDataset")
              .replace("{train}", String(stats["n_train_images"] ?? "—"))
              .replace("{val}", String(stats["n_val_images"] ?? "—"))
              .replace("{cutouts}", String(stats["n_cutouts"] ?? "—"))
              .replace("{real}", String(stats["n_real_full"] ?? "—"))}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="section" style={{ marginTop: 12 }}>
      ...
      {bestF1 && (
        <div className="score-grid">
          <span className="score-label" title={t("metric.f1.tooltip")}>F1</span>
          <span className="score-val best">{bestF1}</span>
          <span className="score-label" title={t("metric.miou.tooltip")}>mIoU</span>
