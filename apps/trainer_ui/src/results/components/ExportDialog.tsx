...
import { exportOnnx, exportCoreML, exportCoreMLUpdatable, exportInstanceOnnx } from "../../api";
import { useI18n, type TranslationKey } from "../../i18n";

type Fmt = "onnx" | "coreml" | "coreml_updatable" | "instance_onnx";
...
type Props = {
  ...
  /** Instance runs export only the serving-registry ONNX (v0.9.8). */
  isInstance?: boolean;
};

type FormatEntry = { id: Fmt; label: string; descKey: TranslationKey; fn: (projectId: string, runId: string) => Promise<unknown> };
...
const INSTANCE_FORMATS: FormatEntry[] = [
  { id: "instance_onnx", label: "ONNX (Serving)", descKey: "results.export.desc.instanceOnnx", fn: exportInstanceOnnx },
];

export default function ExportDialog({ open, projectId, runId, onClose, showToast, isInstance = false }: Props) {
  const { t } = useI18n();
  const formats = isInstance ? INSTANCE_FORMATS : FORMATS;
  const [fmt, setFmt] = useState<Fmt>("onnx");
  const [busy, setBusy] = useState(false);
  if (!open) return null;
  const sel = formats.find((f) => f.id === fmt) ?? formats[0]!;
  const run = async () => {
