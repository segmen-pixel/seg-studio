...
export default React.memo(function PredictionDetail({
  ...
  // Which device actually ran this prediction. The backend has always
  // reported it; nothing displayed it, so an ONNX session that quietly fell
  // back to the CPU provider looked exactly like a healthy GPU run and only
  // showed up as "inference feels slow".
  const deviceLabel = ((): string | null => {
    const raw = predictionScore?.inference_device;
    if (!raw || predictionScore?.presence_only) return null;
    // Two shapes are in the wild: "ort:cuda:1" / "torch:cuda:0" from the
    // paths that record which runtime served the request, and a bare
    // "cuda:0" from older artifacts that only stored the device. Splitting
    // on the first colon regardless turned the bare form into "0 (CUDA)".
    const RUNTIMES = ["ort", "torch"];
    const [head, ...rest] = raw.split(":");
    const runtime = RUNTIMES.includes(head) ? head : null;
    const device = runtime ? rest.join(":") : raw;
    if (!device) return raw.toUpperCase();
    // Rendered as CUDA:0, never "GPU 0": that number is a CUDA ordinal and
    // Windows Task Manager counts adapters, where GPU 0 is often the iGPU.
    return runtime ? `${device.toUpperCase()} (${runtime.toUpperCase()})` : device.toUpperCase();
  })();
  return (
    <>
      {/* ---- Image Prediction ---- */}
      <div className="section" style={{ marginTop: 12 }}>
        ...
        {/* Which device ran it, and how long it took */}
      {(deviceLabel || predictionScore?.inference_ms) && (
        <div
          className="row"
          style={{ justifyContent: "space-between", fontSize: 11, color: "var(--text-secondary)", marginBottom: 8 }}
        >
          <span>{t("results.inferenceDevice")}</span>
          <span>
            {deviceLabel ?? "-"}
            {predictionScore?.inference_ms ? ` · ${Math.round(predictionScore.inference_ms)} ms` : ""}
          </span>
        </div>
      )}
