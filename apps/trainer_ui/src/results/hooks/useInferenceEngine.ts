...
export function useInferenceEngine(
  ...
  async function handleRunInference(targetRunId?: string) {
    ...
    try {
      ...
      s.setInferProgress("");
      onInferStatus?.("");
      s.instanceCacheRef.current.clear();
      ...
      if (s.activeImageId) {
        ...
        s.setPredictionScore(cachedScore && !cachedScore.presence_only ? cachedScore : null);
  ...
  // Pixel histogram (semantic-only artifact — instance runs have no probs)
  useEffect(() => {
    s.setPixelHist(null);
    if (!projectId || !s.activeRunId || s.isInstanceRun) return;
    fetchPixelHistogram(projectId, s.activeRunId, s.predictBackend)
      .then(s.setPixelHist)
      .catch(() => s.setPixelHist(null));
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [projectId, s.activeRunId, s.predictBackend, s.isInstanceRun]);

  // Instance run: load instances.json (readonly) for the active image
  useEffect(() => {
    if (!projectId || !s.activeRunId || !s.activeImageId || !s.isInstanceRun) {
      s.setInstanceData(null);
      return;
    }
    const cacheKey = `${s.activeRunId}:${s.activeImageId}`;
    const cached = s.instanceCacheRef.current.get(cacheKey);
    if (cached !== undefined) {
      s.setInstanceData(cached);
      return;
    }
    // Skip the readonly fetch when prediction status says this image has no artifacts.
    const knownPredicted = s.inferredRunsRef.current.get(s.activeRunId);
    if (knownPredicted !== undefined && !knownPredicted.has(s.activeImageId)) {
      s.setInstanceData(null);
      return;
    }
    let cancelled = false;
    ...
    void (async () => {
      try {
        const data = await fetchRunInstances(projectId, s.activeRunId!, s.activeImageId!, true);
        if (!cancelled) {
          s.instanceCacheRef.current.set(cacheKey, data);
          s.setInstanceData(data);
        }
      } catch {
        if (!cancelled) s.setInstanceData(null);
      }
    })();
    return () => { cancelled = true; };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [projectId, s.activeRunId, s.activeImageId, s.isInstanceRun, s.cacheVersion]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        // A presence-only stub must not satisfy the cache: it exists to carry
        // class chips and every number on it is 0, so short-circuiting here
        // meant the real score.json was never fetched for that image.
        const scoreP = cachedScore && !cachedScore.presence_only
