...
export function useResultsState(
  ...
  const heatmapUrl = useMemo(() => {
    ...
    return null;
  }, [projectId, activeRunId, activeImageId, heatmapMode, heatmapClassId, predictBackend, ttaEnabled, confidenceThreshold, ppMinArea, ppMaxArea]);
  ...
  const effectiveMaskIndex = useMemo(() => {
    ...
    return result;
  }, [maskIndex, confidenceIndex, confidenceThreshold]);
  ...
  const regionCounts = useMemo(() => {
    if (!showCount || effectiveMaskIndex.length === 0 || width === 0) return new Map<number, number>();
    return countRegionsPerClass(effectiveMaskIndex, width, height);
  }, [showCount, effectiveMaskIndex, width, height]);
