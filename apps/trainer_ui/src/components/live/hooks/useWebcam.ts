...
export function useWebcam(
  ...
  const connectCamera = useCallback(async () => {
    ...
    try {
      // Configure
      await fetch(`${API_ORIGIN}/v2/camera/config`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(cameraConfig),
      });
      // Start
      const res = await fetch(`${API_ORIGIN}/v2/camera/start`, { method: "POST" });
      if (!res.ok) throw new Error(await res.text());
      setCameraState("PREVIEW");

      // Open camera WS for preview + results
      const wsUrl = `${API_ORIGIN.replace(/^http/, "ws")}/ws/v2/camera`;
