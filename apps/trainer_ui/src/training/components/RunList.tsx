...
import { formatApiDate, resolveRunStart } from "../../time";
          ...
          >
            <div>
              ...
              {(() => {
                ...
                const { date: d, isStart } = resolveRunStart(run.started_at, run.created_at);
                if (!d) return null;
                const hh = String(d.getHours()).padStart(2, "0");
                const mm = String(d.getMinutes()).padStart(2, "0");
                // started_at is the moment training began; a queued run, and any
                // row written before that column existed, has none, and then this
                // is the creation time and says so rather than claiming a start.
                const label = isStart
                  ? (lang === "ja" ? "学習開始" : "Started")
                  : (lang === "ja" ? "作成" : "Created");
                const iso = isStart ? run.started_at : run.created_at;
                return (
                  <span
                    style={{ marginLeft: 6, fontSize: 11, opacity: 0.6 }}
                    title={`${label}: ${formatApiDate(iso)}`}
                  >
                    {hh}:{mm}
                  </span>
                );
