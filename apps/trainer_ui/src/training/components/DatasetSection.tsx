...
export default React.memo(function DatasetSection({ form, hasCuda }: DatasetSectionProps) {
  ...
  return (
    <>
      ...
      <label className="hp-cat-input" data-desc={t("training.minArea.desc")}>{t("training.minArea")}</label>
      <div className="row training-inline-controls">
        <select aria-label="Mask noise filter mode" value={form.trainUseAutoMinArea ? "auto" : "manual"} onChange={(e) => form.setTrainUseAutoMinArea(e.target.value === "auto")}>
          <option value="auto">{t("common.auto")}</option>
          <option value="manual">{t("common.manual")}</option>
        </select>
        <NumberField aria-label="Mask noise filter value" integer min={1} max={10000} value={form.trainMinArea} onCommit={form.setTrainMinArea} disabled={form.trainUseAutoMinArea} />
      </div>
