...
import { resolveRunStart } from "../../time";
      ...
      {bestRun ? (
        <>
          ...
          <div className="summary-zone-metrics">
            ...
            {(bestRun.started_at || bestRun.created_at) && (() => {
              // This row said "Started" while showing created_at, which for a run
              // that waited for a GPU is when it was queued. It now says which
              // of the two it is showing.
              const { date: d, isStart } = resolveRunStart(bestRun.started_at, bestRun.created_at);
              if (!d) return null;
              ...
              return (
                <div className="summary-kv-row">
                  <span className="summary-kv-label">{label}</span>
