...
// "nano" was dropped in v0.9.8 (not accurate enough for exact counting);
// existing nano checkpoints stay usable for prediction.
export type InstanceModelSize = "small" | "medium" | "large";
export const INSTANCE_MODEL_SIZES: InstanceModelSize[] = ["small", "medium", "large"];

export const INPUT_SIZE_PRESETS = [128, 192, 256, 320, 384, 512] as const;
export const PATCH_SIZE_PRESETS = [128, 256, 512] as const;

// Instance tiling. The detector's input is fixed (384 small/nano, 432
// medium/large), so a patch is what decides the scale an object reaches the
// model at -- and the same value is used for inference, because a mismatch
// produces no error, only a quietly wrong count. 0 turns tiling off and
// composes whole plates. 768 is the default: it doubles the small model's
// input, and an image smaller than it is a single padded tile, so projects
// with small images behave exactly as they did before tiling existed.
export const INSTANCE_PATCH_SIZE_PRESETS = [0, 384, 512, 768, 1024] as const;
...
export type TrainFormState = {
  ...
  trainArch: "simpleunet" | "stdc";
  trainLossType: "auto" | "ce" | "focal" | "lovasz";
  trainDeepSupervision: boolean;
  trainFrequencyMap: boolean;
  trainUseAutoMinArea: boolean;
  trainMinArea: number;
  // Instance mode (v0.9.8): synthesis + rfdetr settings
  trainInstanceModelSize: InstanceModelSize;
  trainInstancePatchSize: number;
  trainInstanceNTrain: number;
  trainInstanceNVal: number;
  trainInstanceObjectsMin: number;
  trainInstanceObjectsMax: number;
  trainInstanceStackPairProb: number;
  trainInstanceSeed: number;
  trainInstanceAreaBandMin: number;
  trainInstanceAreaBandMax: number;
  // UI
...
export function useTrainForm(): TrainFormReturn {
  ...
  const [trainArch, setTrainArch] = useState<"simpleunet" | "stdc">("simpleunet");
  ...
  // Auto is what this form has always sent: postprocess_min_area 0, which the
  // backend reads as "measure the threshold from the masks" rather than as
  // "no filtering". Manual starts at 1, which removes nothing, so changing
  // the mode cannot delete anything on its own.
  const [trainUseAutoMinArea, setTrainUseAutoMinArea] = useState(true);
  const [trainMinArea, setTrainMinArea] = useState(1);


  // Instance mode (v0.9.8)
  const [trainInstanceModelSize, setTrainInstanceModelSize] = useState<InstanceModelSize>("small");
  const [trainInstancePatchSize, setTrainInstancePatchSize] = useState(768);
  const [trainInstanceNTrain, setTrainInstanceNTrain] = useState(300);
  const [trainInstanceNVal, setTrainInstanceNVal] = useState(80);
  const [trainInstanceObjectsMin, setTrainInstanceObjectsMin] = useState(4);
  const [trainInstanceObjectsMax, setTrainInstanceObjectsMax] = useState(8);
  const [trainInstanceStackPairProb, setTrainInstanceStackPairProb] = useState(0.55);
  const [trainInstanceSeed, setTrainInstanceSeed] = useState(42);
  const [trainInstanceAreaBandMin, setTrainInstanceAreaBandMin] = useState(0); // 0 = auto
  const [trainInstanceAreaBandMax, setTrainInstanceAreaBandMax] = useState(0); // 0 = auto
  ...
  function buildPayload(
    ...
    if (!trimmedName) {
      // Local calendar date, not toISOString() (which is UTC): in JST a run
      // started before 09:00 would otherwise be named with yesterday's date,
      // and the daily sequence counter would roll over at 09:00 instead of 00:00.
      const _now = new Date();
    ...
    // Auto omits the key, as the other auto knobs here do: the backend's
    // default of 0 is what makes it measure the 6-sigma threshold from the
    // training masks. Which is also why manual is clamped to 1 -- a 0 that
    // reached the API would read as auto, the opposite of what someone typing
    // it into a minimum-area box is asking for.
    if (!trainUseAutoMinArea) {
      const minArea = Number.isFinite(trainMinArea) ? Math.round(trainMinArea) : 1;
      payload.postprocess_min_area = Math.max(1, Math.min(10000, minArea));
    }
    ...
    if (trainingMode === "instance") {
      // The backend rejects instance runs with k-fold / iterative — force off
      // rather than surfacing a 400 for stale semantic-form state.
      payload.k_folds = 1;
      payload.iterative_mode = false;
      payload.instance_model_size = trainInstanceModelSize;
      // 0 is meaningful (tiling off), so clamp rather than fall back to a
      // default the user did not choose.
      payload.instance_patch_size = Math.max(0, Math.min(4096, Math.round(trainInstancePatchSize) || 0));
      payload.instance_n_train = Math.max(8, Math.min(5000, Math.round(trainInstanceNTrain) || 300));
      payload.instance_n_val = Math.max(2, Math.min(1000, Math.round(trainInstanceNVal) || 80));
      const objMin = Math.max(1, Math.min(64, Math.round(trainInstanceObjectsMin) || 4));
      payload.instance_objects_min = objMin;
      payload.instance_objects_max = Math.max(objMin, Math.min(64, Math.round(trainInstanceObjectsMax) || 8));
      payload.instance_stack_pair_prob = Math.max(0, Math.min(1, trainInstanceStackPairProb));
      payload.instance_seed = Math.max(0, Math.round(trainInstanceSeed) || 0);
      if (trainInstanceAreaBandMin > 0 && trainInstanceAreaBandMax > 0) {
        payload.instance_area_band_min = Math.round(trainInstanceAreaBandMin);
        payload.instance_area_band_max = Math.max(
          Math.round(trainInstanceAreaBandMin), Math.round(trainInstanceAreaBandMax));
      }
    }
    return payload;
  }
  ...
  return {
    ...
    trainUseAutoMinArea, setTrainUseAutoMinArea,
    trainMinArea, setTrainMinArea,
    trainInstanceModelSize, setTrainInstanceModelSize,
    trainInstancePatchSize, setTrainInstancePatchSize,
    trainInstanceNTrain, setTrainInstanceNTrain,
    trainInstanceNVal, setTrainInstanceNVal,
    trainInstanceObjectsMin, setTrainInstanceObjectsMin,
    trainInstanceObjectsMax, setTrainInstanceObjectsMax,
    trainInstanceStackPairProb, setTrainInstanceStackPairProb,
    trainInstanceSeed, setTrainInstanceSeed,
    trainInstanceAreaBandMin, setTrainInstanceAreaBandMin,
    trainInstanceAreaBandMax, setTrainInstanceAreaBandMax,
