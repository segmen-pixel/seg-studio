...
export function useDrawingEvents(
  ...
  function handlePointerDown(event: React.PointerEvent) {
    ...
    if ((tool === "sam" || tool === "sambox") && projectId && activeImageId) {
      ...
      // Background (negative) click prompts are opt-in and OFF by default —
      // the default annotation flow uses positive clicks only. Right-click is
      // ignored entirely when disabled, so it never silently degrades into an
      // extra positive point.
      const negativeEnabled = localStorage.getItem("seg_sam_negative_points") === "1";
      if (event.button === 2 && !negativeEnabled) return;
