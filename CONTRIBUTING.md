...
| Add a Python unit test | `tests/` or `apps/trainer_api/tests/` |
...
| `apps/trainer_api/requirements-cu128.in` / `.txt` | Same pair for the parallel Blackwell (CUDA 12.8) lockfile. |
...
`requirements-cu128.txt` is a **parallel lockfile** that bumps the torch
family to `2.11.0` (installed from the CUDA 12.8 wheel index) to add
Blackwell (sm_120 / RTX 5090 / B200) support. All non-torch dependencies
are identical between the two lockfiles. See
[`docs/BLACKWELL_MIGRATION.md`](docs/BLACKWELL_MIGRATION.md)
for the setup procedure and verification checklist.

| Lockfile | torch | CUDA | arch_list covers | Status |
|---|---|---|---|---|
| `requirements.txt`        | 2.6.0  | 12.4 | sm_60 – sm_90  | Production (Ampere / Hopper) |
| `requirements-cu128.txt`  | 2.11.0 | 12.8 | sm_75 – sm_120 | Validated for Blackwell |

Newer PyTorch wheels are backwards-compatible with older supported GPUs,
so the cu128 variant also runs on 3080Ti / 3090. Choose the lockfile
...
lockfiles record canonical PyPI versions (no `+cu128` local-version
